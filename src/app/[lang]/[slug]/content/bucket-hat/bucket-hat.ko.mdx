...
<img src="/bucket-hat/images/cone-development.png" />
...
- Connect each particle to the one above it (vertical connection).
- Connect each particle to the one on the left (horizontal connection).
- If the particle is at the leftmost edge, connect it to the rightmost particle to simulate a seamless wrap (useful for cylindrical shapes like a hat brim).
...
There’s still a lot I could improve—more shape options, better physics, maybe even different hat styles. But for now, it does what I initially envisioned.
